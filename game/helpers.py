...
def calculate_distance_boundaries(table, player, direction):
    ...
    while r:
        if direction == "left":
            ...
            if player_x >= table[index_tile_x1_up]["x1"] and player_x <= table[index_tile_x1_up]["x2"]:
                distance_top_1 = player_y - table[index_tile_x1_up]["y2"]
            else: index_tile_x1_up += 1

            if player_x+32 >= table[index_tile_x2_up]["x1"] and player_x+32 <= table[index_tile_x2_up]["x2"]:
            ...
            if player_x >= table[index_tile_x1_down]["x1"] and player_x <= table[index_tile_x1_down]["x2"]:
                distance_bottom_1 = table[index_tile_x1_down]["y1"] - player_y+32
            else: index_tile_x1_down += 1

            if player_x+32 >= table[index_tile_x2_down]["x1"] and player_x+32 <= table[index_tile_x2_down]["x2"]:
...
# DOWN AND UP DOESN'T WORKS WELL
